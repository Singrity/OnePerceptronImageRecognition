...
class Perceptron:
    ...
    def train(self, X, Y, max_epochs=50, error_threshold=0.0, ui_callback=None):
        ...
        for epoch in range(max_epochs):
            ...
            self.training_log.append({'epoch': epoch, 'error': total_error, 'mis': misclassified})
            if ui_callback: ui_callback(epoch, total_error, misclassified)
            if total_error <= error_threshold: break
...
class MNISTApp:
    ...
    def _build_ui(self):
        ...
        ttk.Button(frm, text="📥 Загрузить MNIST", command=self.load_mnist).grid(row=0, column=0, padx=5, pady=5)
        
        ttk.Label(frm, text="η:").grid(row=0, column=1, padx=5)
        self.ent_lr = ttk.Entry(frm, width=6); self.ent_lr.insert(0, "0.1"); self.ent_lr.grid(row=0, column=2)
        
        ttk.Label(frm, text="Эпохи:").grid(row=0, column=3, padx=5)
        self.ent_epochs = ttk.Entry(frm, width=6); self.ent_epochs.insert(0, "50"); self.ent_epochs.grid(row=0, column=4)
        
        self.btn_train = ttk.Button(frm, text="🎓 Обучить", command=self.start_training, state="disabled")
        self.btn_train.grid(row=0, column=5, padx=5)
        self.btn_test = ttk.Button(frm, text="🔍 Тест", command=self.run_test, state="disabled")
        ...
        self.tab_train = ttk.Frame(nb); self.tab_test = ttk.Frame(nb)
        nb.add(self.tab_train, text="📊 Обучение"); nb.add(self.tab_test, text="🎯 Тестирование")
        
        ttk.Label(self.tab_train, text="Примеры из выборки:").pack(anchor="w", padx=10, pady=5)
        self.frm_samples = ttk.Frame(self.tab_train); self.frm_samples.pack(fill="x", padx=10, pady=5)
        self.log_txt = tk.Text(self.tab_train, height=12); self.log_txt.pack(fill="both", expand=True, padx=10, pady=5)
        ...
        self.frm_test_imgs = ttk.LabelFrame(self.tab_test, text="Тестируемые изображения")
        self.frm_test_imgs.pack(fill="x", padx=10, pady=5)
        self.res_txt = tk.Text(self.tab_test, height=14); self.res_txt.pack(fill="both", expand=True, padx=10, pady=5)
    ...
    def load_mnist(self):
        ...
        self.lbl_status.config(text=f"✓ MNIST загружен | Всего: {len(y_tr)} | Семёрок: {n7} | Не-семёрок: {len(y_tr)-n7}", foreground="green")
        self._show_samples(x_tr[:10], y_tr[:10])
        
    def _show_samples(self, imgs, labs):
        for w in self.frm_samples.winfo_children(): w.destroy()
        for i in range(10):
            img = Image.fromarray(imgs[i]).resize((40, 40))
            ph = ImageTk.PhotoImage(img)
            lbl = ttk.Label(self.frm_samples, image=ph); lbl.image = ph; lbl.grid(row=0, column=i, padx=2)
            txt = ttk.Label(self.frm_samples, text=str(labs[i]), foreground="green" if labs[i]==7 else "red")
            txt.grid(row=1, column=i, padx=2)
            
    def _update_log(self, epoch, err, mis):
        self.log_txt.insert("end", f"Эпоха {epoch:2d} | Ошибка: {err:4.0f} | Неверно: {mis:4d}\n")
        self.log_txt.see("end"); self.root.update_idletasks()
    ...
    def start_training(self):
        ...
        np.random.shuffle(idx7); np.random.shuffle(idx0)
        idx = np.concatenate([idx7[:500], idx0[:1500]])
        ...
        self.btn_train.config(state="normal"); self.btn_test.config(state="normal")
    ...
    def _plot_curve(self, log):
        ...
        if log:
            ...
            self.ax.plot(epochs, errors, 'b-o', lw=2, markersize=4, label="Суммарная ошибка")
            self.ax.set_xlabel("Эпоха"); self.ax.set_ylabel("Ошибка")
    ...
    def run_test(self):
        ...
        for w in self.frm_test_imgs.winfo_children(): w.destroy()
    ...
    def _show_weights(self):
        ...
        ttk.Label(self.frm_test_imgs, text="Визуализация весов сети").grid(row=2, column=0, columnspan=10)
